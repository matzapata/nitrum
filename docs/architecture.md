...
The control-plane supervises `nitro-cli run-enclave` and does **not** treat a successful launch as stable until the enclave stays listed by `describe-enclaves` for a short settle window. If it disappears earlier, the supervisor logs a crash-loop warning (`exited_before_stable`, `uptime_ms`, `consecutive_fast_crashes`) and **does not reset** restart backoff (exponential delay, capped). Ingress readiness is left to the NLB probing `/.well-known/enclave/status` on **:443**.
...
- **Emission.** Both binaries call `telemetry::init` at process start so structured stdout logs are available immediately. The data-plane attaches OTLP later (via `TelemetryGuard::enable_otlp`) once IMDS/config resolve the collector endpoint and egress is up—networking, config, and IMDS failures therefore still appear in CloudWatch via the container log driver even when OTLP is not ready yet. When OTLP is configured, traces, metrics, and logs export over OTLP/gRPC; binaries return `ExitCode` so the telemetry guard Drop flushes exporters (including attested KMS AccessDenied fatals). Telemetry payloads stay backend-neutral; the deployed data-plane only uses IMDS to discover the parent host address for its default collector endpoint.
- **Instrumented paths.** HTTP request count/latency on the ingress (`data-plane.ingress`) and crypto API (`data-plane.crypto-api`) routers (by matched route template, method, status class); KMS call latency and errors (attested Decrypt AccessDenied is logged as a first-class once-per-boot error with key id / ImageSha384 context); ACME issue/renew events and a certificate-expiry gauge; and control-plane enclave (re)start counts (`started`, `failed`, `exited_before_stable`). Platform telemetry is tagged with `nitrum.component=core` and `service.namespace=nitrum`.
